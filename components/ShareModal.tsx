...
const ShareModal: React.FC<ShareModalProps> = ({ isOpen, onClose, url, title }) => {
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
      ...
      <div className="bg-white rounded-[2.5rem] w-full max-w-sm p-8 relative z-10 shadow-2xl animate-fade-in border border-gray-100 gpu-accelerated overflow-hidden">
        ...
        <button 
          ...
          className={`w-full py-5 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all duration-500 flex items-center justify-center gap-3 active:scale-95 ${
            isCopied ? 'bg-green-500 text-white shadow-xl shadow-green-200' : 'bg-primary text-white hover:bg-accent'
          }`}
        >
          {isCopied ? (
            <>
              <svg className="w-4 h-4 animate-[subtlePulse_0.5s_ease-in-out]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
              Link Copiado!
            </>
          ) : (
            <>
