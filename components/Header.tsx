...
import { searchGlobal } from '../lib/mcpClient';
...
const Header: React.FC = () => {
  ...
  return (
    <>
            ...
            <div className="flex items-center space-x-3 xl:space-x-5">
              {navLinks.map((link) => (
                <Link
                  key={link.path}
                  to={link.path}
                  className={`text-[8.5px] xl:text-[10px] font-black uppercase tracking-[0.15em] xl:tracking-[0.2em] transition-[color,transform] hover:text-accent relative py-2 whitespace-nowrap ${
      ...
      <div 
        className={`fixed inset-0 z-[1000] bg-white transition-[transform,opacity] duration-700 [transition-timing-function:cubic-bezier(0.16,1,0.3,1)] flex flex-col will-change-composite gpu-accelerated ${
      ...
      <div 
        className={`lg:hidden fixed inset-0 z-[999] bg-white transition-[transform,opacity,visibility] duration-700 [transition-timing-function:cubic-bezier(0.16,1,0.3,1)] will-change-transform gpu-accelerated ${
          isMenuOpen ? 'translate-y-0 opacity-100 visible pointer-events-auto' : 'translate-y-full opacity-0 invisible pointer-events-none'
        }`}
      >
        <div className="flex flex-col h-full w-full bg-white pt-24 md:pt-32 pb-10 px-8 md:px-16 overflow-hidden">
          ...
          <nav className="flex flex-col flex-grow justify-center space-y-3 md:space-y-4 text-center">
            {navLinks.map((link, index) => (
              <Link
                key={link.path}
                to={link.path}
                className={`text-2xl sm:text-3xl md:text-4xl font-serif font-black uppercase tracking-tighter transition-[transform,opacity,color] duration-500 block active:scale-95 ${
          ...
          <div 
            className={`mt-auto pt-8 border-t border-gray-100 transition-[transform,opacity] duration-700 ${
              isMenuOpen ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
            }`} 
